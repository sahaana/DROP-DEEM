...
\minihead{Step 1: Progressive Sampling (\S\ref{subsec:psample}, Alg~\ref{alg:DROP} L\ref{eq:sample}, Fig 4A)}
...
\minihead{Step 2: Transform Evaluation (\S\ref{subsec:teval}, Alg~\ref{alg:DROP} L\ref{eq:evaluate}, Fig 4B)} 
...
\minihead{Step 3: Progress Estimation (\S\ref{subsec:pest}, Alg~\ref{alg:DROP} L\ref{eq:estimate}, Fig 4C)} 
...
\minihead{Step 4: Cost-Based Optimization (\S\ref{subsec:opt}, Alg~\ref{alg:DROP} L\ref{eq:optimize}, Fig 4D)} 
...
\label{subsec:teval}
DROP must accurately and efficiently evaluate this iteration's performance with respect to a metric of interest \red{over the entire dataset}. 
