...
\red{Similarity search is a repeated-query workload performed over data types including images, documents and time series~\cite{keogh-study,lsh}.
We use similarity search as a running case study given its popularity and the large amount of research in the space.
...
As input, consider an input dataset ($X$), target metric preservation ($B$; e.g., $TLB \geq .99$), and optional downstream runtime as a function of dimensionality ($\mathcal{C}_\mvar(\dvar)$ for an $\mvar\times \dvar$ matrix).  
